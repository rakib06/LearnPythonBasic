import operator  # python build in operator used for dictionary
...
class MyTextFile:
    ...
    def sequence_from_dict(dict):
        sorted_dict = MyTextFile.key_sorted_dict(dict)
        sequence = []
    ...
    def sequence_and_cost_convert_from_txt(path, file_name, dept):
        ...
        dict = MyTextFile.key_sorted_dict(unique_dict)
        sequence = MyTextFile.sequence_from_dict(dict)
